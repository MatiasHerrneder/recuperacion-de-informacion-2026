...
def retriever(index, queries: list[dict], qrels: dict[str, set[str]], results_per_query: int | None = None, model: str = MODEL):
    try:
        ...
        # summary_rows = []
        metrics = {}
        metrics[f"P@{results_per_query}"] = 0
        metrics["MAP"] = 0
        metrics[f"nDCG@{results_per_query}"] = 0
        metrics[f"R@{results_per_query}"] = 0
        ...
        for q in queries:
            ...
            with open(out_file, "w", encoding="utf-8") as f:
                ...
                for rank, (_, row) in enumerate(query_res.iterrows(), start=1):
                    if results_per_query and rank > results_per_query: break
                    ...
                    precision_q.append(retrieved_relevant / rank)
                    recall_q.append(retrieved_relevant / len(relevant_docs)) if len(relevant_docs) > 0 else recall_q.append(0)

                    f.write(f"{rank:<6} {docno:<12} {score:<12.4f} {'1' if is_relevant else '0'}\n")


            k = min(results_per_query, len(query_res)) if results_per_query else len(query_res)
            ...
            for i in range(1, ideal_hits + 1):
                idcg += 1 if i == 1 else 1 / math.log2(i)
            ndcg = dcg / idcg if idcg > 0 else 0

            metrics[f"P@{results_per_query}"] += precision
            metrics["MAP"] += avg_precision
            metrics[f"nDCG@{results_per_query}"] += ndcg
            metrics[f"R@{results_per_query}"] += recall
            
            metrics["precision"][qid] = precision_q
            metrics["recall"][qid] = recall_q

            metrics["per_query"]["P"].append(precision)
            metrics["per_query"]["AP"].append(avg_precision)
            metrics["per_query"]["NDCG"].append(ndcg)
            metrics["per_query"]["R"].append(recall)

        metrics[f"P@{results_per_query}"] /= len(queries)
        metrics["MAP"] /= len(queries)
        metrics[f"nDCG@{results_per_query}"] /= len(queries)
        metrics[f"R@{results_per_query}"] /= len(queries)

        max_rank = max(len(v) for v in metrics["precision"].values())

        for i in range(max_rank):
            p_values = [v[i] for v in metrics["precision"].values() if i < len(v)]
            r_values = [v[i] for v in metrics["recall"].values() if i < len(v)]
            metrics["avgP"].append(sum(p_values) / len(p_values))
            metrics["avgR"].append(sum(r_values) / len(r_values))
        
        print(metrics)
        with open(os.path.join(out_dir, "metrics.txt"), "w", encoding="utf-8") as r:
            r.write(f"Metric values with (k) results per query = {results_per_query}:\n")
            for metric, value in metrics.items():
                r.write(f"{metric}: {value}\n")
...
def main():
    ...
    retriever(index, queries, qrels, results_per_query=RESULTS_PER_QUERY)
