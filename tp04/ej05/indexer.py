...
def index_bsbi(corpus_path: str, memory_limit: int = 10, output_dir: str = "output", stop_words_path: str | None = None):
    ...
    with open(os.path.join(output_dir, INDEX_SUBPATH), 'wb') as index, open(os.path.join(output_dir, SKIPLIST_SUBPATH), 'wb') as skips_file:
        for term_id_actual in sorted(term_to_id.values()):
            ...
            if posting_lists:
                ...
                skip_interval = max(1, int(math.sqrt(len(posting_lists))))

                for i in range(0, len(posting_lists), skip_interval):
                    if i + skip_interval < len(posting_lists):
                        if term_id_actual not in skip_index:
                            skip_index[term_id_actual] = [skips_file.tell(), 0]
                        skip_index[term_id_actual][1] += 1
                        skips_file.write(struct.pack('>III', term_id_actual, i, i + skip_interval))
    ...
    pickle.dump(skip_index, open(os.path.join(output_dir, SKIP_INDEX_SUBPATH), 'wb'))
